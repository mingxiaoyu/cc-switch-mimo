...
impl McpApps {
    ...
    pub fn set_enabled_for(&mut self, app: &AppType, enabled: bool) {
        match app {
            ...
            AppType::MimoCode => self.mimo = enabled,
        }
    }
    ...
    pub fn is_empty(&self) -> bool {
        !self.claude && !self.codex && !self.gemini && !self.opencode && !self.hermes && !self.mimo
    }
}
...
pub struct McpRoot {
    ...
    /// MimoCode MCP 配置（实际使用 mimocode.jsonc）
    #[serde(default, skip_serializing_if = "McpConfig::is_empty")]
...
impl AppType {
    ...
    /// - Additive mode (true): All providers are written to live config (OpenCode, OpenClaw, Hermes, MimoCode)
...
impl FromStr for AppType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match normalized.as_str() {
            ...
            "mimo" => Ok(AppType::MimoCode),
            other => Err(AppError::localized(
                "unsupported_app",
                format!("不支持的应用标识: '{other}'。可选值: claude, claude-desktop, codex, gemini, opencode, openclaw, hermes, mimo。"),
                format!("Unsupported app id: '{other}'. Allowed: claude, claude-desktop, codex, gemini, opencode, openclaw, hermes, mimo."),
            )),
        }
    }
}
...
impl MultiAppConfig {
    ...
    pub fn load() -> Result<Self, AppError> {
        ...
        // 确保 mimo 应用存在
        if !config.apps.contains_key("mimo") {
            config
                .apps
                .insert("mimo".to_string(), ProviderManager::default());
            updated = true;
        }
