...
use crate::mimocode_config::{get_mimocode_data_dir, get_mimocode_db_path};
...
/// Return the MimoCode JSON storage directory.
pub(crate) fn get_mimocode_storage_dir() -> PathBuf {
    get_mimocode_data_dir().join("storage")
}
...
fn scan_sessions_json() -> Vec<SessionMeta> {
    let storage = get_mimocode_storage_dir();
...
fn scan_sessions_sqlite() -> Vec<SessionMeta> {
    ...
    for row in iter.flatten() {
        ...
        sessions.push(SessionMeta {
            ...
            resume_command: Some(format!("mimocode -s {session_id}")),
        });
    }
    sessions
}
...
/// Extract text content from a part JSON value.
...
mod tests {
    ...
    fn delete_session_sqlite_removes_session() {
        let temp = tempdir().expect("tempdir");
        let db_path = temp.path().join("mimocode.db");
        // 让 get_mimocode_db_path() 指向 tempdir，使 canonicalize 能成功
        std::env::set_var("MIMOCODE_DB", db_path.to_str().unwrap());
        let conn = Connection::open(&db_path).expect("open sqlite db");
        create_sqlite_schema(&conn);
        ...
        std::env::remove_var("MIMOCODE_DB");
    }
    ...
    fn delete_session_sqlite_rejects_foreign_db_path() {
        ...
        // Create expected db at the real mimocode data dir location
        let expected_db_path = get_mimocode_db_path();
        if let Some(parent) = expected_db_path.parent() {
            let _ = std::fs::create_dir_all(parent);
        }
        let _ = Connection::open(&expected_db_path).expect("create expected sqlite db");
